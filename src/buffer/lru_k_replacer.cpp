...
auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool {
    ...
    for(auto temp_it : temp_pool_) {
        if(node_store_[temp_it]->GetEvictable() == false) {
            continue;
        }
        *frame_id = temp_it;
    ...
    for(auto it = cache_pool_.begin(); it != cache_pool_.end(); it++) {
        auto cache_it = *it;
        if(node_store_[cache_it.first]->GetEvictable() == false) {
            continue;
        }
...
void LRUKReplacer::RecordAccess(frame_id_t frame_id, [[maybe_unused]] AccessType access_type) {
    std::lock_guard<std::mutex> lock(latch_);
    BUSTUB_ASSERT(static_cast<size_t>(frame_id) <= replacer_size_, "frame id is invalid(larger than replacer_size_)");
    ++current_timestamp_;
    ...
    if(temp_it != temp_pool_map_.end()) {
        ...
        if(backk != 0) {
            ...
            auto insert_pos = std::upper_bound(cache_pool_.begin(), cache_pool_.end(), new_pair, [](const std::pair<frame_id_t, size_t>& a, 
...
void LRUKReplacer::SetEvictable(frame_id_t frame_id, bool set_evictable) {
    ...
    if(frame_ptr->GetEvictable() == true && set_evictable == false) {
        frame_ptr->SetEvictable(set_evictable);
        evictable_size_--;
    } else if(frame_ptr->GetEvictable() == false && set_evictable == true) {
        frame_ptr->SetEvictable(set_evictable);
        evictable_size_++;
    }
}
...
void LRUKReplacer::Remove(frame_id_t frame_id) {
    ...
    auto frame_ptr = frame_it->second;
    BUSTUB_ASSERT(frame_ptr->GetEvictable() == false, "Remove is called on a non-evictable frame");
    auto temp_it = temp_pool_map_.find(frame_id);
    if(temp_it != temp_pool_map_.end()) {
        temp_pool_.erase(temp_it->second);
        temp_pool_map_.erase(temp_it);
